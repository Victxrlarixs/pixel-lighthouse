...
export function initApp() {
  ...
  $isScanning.subscribe((scanning) => {
    if (!btnRun) return;
    btnRun.innerHTML = scanning ? '<i class="fas fa-spinner fa-spin"></i>' : '<i class="fas fa-bolt"></i>';
  });
  ...
  function updateBar(bar: HTMLElement, val: HTMLElement, p: number) {
    ...
    bar.style.background = valP >= 90 ? 'var(--accent-green)' : (valP >= 50 ? 'var(--accent-yellow)' : 'var(--accent-red)');
  }
  ...
  if (btnRun && urlInput) {
    btnRun.addEventListener('click', async () => {
      ...
      const iv = setInterval(() => {
        progress += 1.5; if (progress > 95) progress = 95;
      ...
      try {
        ...
        clearInterval(iv);
        if (progressFill) progressFill.style.backgroundColor = 'var(--accent-red)';
      }
    });
  ...
  if (scoreSlider) {
    scoreSlider.addEventListener('input', () => sim.forceScore(parseInt(scoreSlider.value)));
  }
